...
$query = "select * from registration where email ='".$email."' LIMIT 1 ";
	...
	foreach($run as $key=> $use)
	
	$user_id=$use['id'];$user_email=$use['email'];
...
$status = '0';
 ...
 if($product_offer > '0')
 {
		 	...
		 	$discounttoltal =$cost*$product_offer/$number;
$offer_amount=$cost-$discounttoltal;
$offer_amount=round($offer_amount);
 	$final=$offer_amount*$quantity;
 }
 else{
 	$final=$cost*$quantity;
 }
 ...
 {
  echo 	$product_sku=$_SESSION['order_id'];
 }
 else
 {
 	
 $random1_digit=rand(000000,999999);
 $date=date('Ymd'); $date1=date('Y-m-d');
 $nameg='CBP-';
 $product_sku=$nameg.$date.$random1_digit;
 
 $_SESSION['order_id']=$product_sku;
 }
 ...
 if($product_offer=="")
 {
   ...
   $co='';$cashondelivery='cashondelivery';
$table = "order_details";$ac='';

  $in="INSERT INTO `order_details`(  `order_id`, `customer_user_id`, `product_name`, `sku`, `unit_cost`, `quantity`, `amount`, `shipping_method`, `role`, `order_date`,`offer` ) VALUES ( '$product_sku','$user_id','$product_name','$co','$cost','$quantity','$final','$cashondelivery','$user_email','$date1','$product_offer' )";
$m=mysqli_query($conn,$in);

//$field_value=array($product_sku,$user_id,$product_name,$product_name,$co,$cost,$quantity,$final,$cashondelivery,$user_email,$date1,$product_offer );
//$field =array('order_id', 'customer_user_id', 'product_name', 'sku', 'unit_cost', 'quantity', 'amount', 'shipping_method', 'role', 'order_date', 'offer' );
//$res = $con->insert($field_value,$table,$field);
  if($res)
  {
  	

 //echo'<script>location.assign("creat.php")</script>';
	}  }
	
	if(isset($_POST['btnfinal']))
	{ $Orderid=$_SESSION['order_id'];
	$query4= "SELECT * from order_details where order_id='$Orderid'";
 ...
 $totalk=$_SESSION['total'];
 $total=$total1;   $email  = $_POST['email'];
 $query = "select * from registration where email ='".$email."' LIMIT 1 ";
	$run = mysqli_query($conn,$query);
	foreach($run as $key=> $use)
	
	$user_id=$use['id'];$email=$use['email'];$mobile_no=$use['mobile'];
 ...
 $dis='';
	...
	$email=$_POST['email'];
												...
												$phone=$use['mobile'];
													...
													$date=date('d-m-y');
			     $query="INSERT into customer_detail (fname,lname,mobile_no,pincode,address,email,country,state,company,ondate) VALUES('$first','$last','$phone','$postc','$street','$email','$country','$city','$company','$date')";
			$run=mysqli_query($conn,$query);
 ...
 //$total= $_SESSION['total_amount'];
  $select="SELECT * from tbl_tax order by tax_id desc LIMIT 1";
                  ...
                  foreach($run as $key=> $tax)
                   $number='100';  
                  $taxn=$tax['vat'];
		 	$discounttoltal =$total*$taxn/$number;
		 	$Orderdate=date('Y-m-d');
		 	$offerc=$discounttoltal+$total;
           $query8="INSERT INTO order_summary (order_id,email_id,mobile_no,order_date,total_amount,subtotal,coupondiscount,process_type,payment_type,remarks,invoice_status,invoiceno,feed) VALUES('$Orderid','$email','$mobile_no','$Orderdate','$offerc','$total','$dis','$type','$paymentmethod','','','0','0')";
...
unset($_SESSION['order_id']);
 ...
 echo '<script>window.location.assign("../thankyou.php?code='.$Orderid.'")</script>';	
	}
	?>
              ...
              <div class="col-md-12">
                <div class="panel widget">
                  <div class="panel-heading vd_bg-grey">
                    <h3 class="panel-title"> <span class="menu-icon"> <i class="fa fa-bar-chart-o"></i> </span> Add Product Categories  </h3>
                  </div>
                ...
                <div class="form-group">
                        <label class="col-sm-2 control-label">Add Product Name</label>
                        <div class="col-sm-7 controls">
                          <input class="width-70" type="text" placeholder="Input"  name="product_name" >
                        </div>
                      </div>
                       
                    
                       <div class="form-group">
                        <label class="col-sm-2 control-label">Add Product Cost</label>
                        <div class="col-sm-7 controls">
                          <input class="width-70" type="text" placeholder="Input"  name="cost" >
                        </div>
                      </div>
                        <div class="form-group">
                        <label class="col-sm-2 control-label">Add Product Quantity</label>
                        <div class="col-sm-7 controls">
                          <input class="width-70" type="text" placeholder="Input"  name="quantity" > 
                          <input class="width-70" type="hidden" placeholder="Input" value="<?php echo $_GET['id'];?>"  name="id"  >
                            <input class="width-70" type="hidden" placeholder="Input" value="<?php echo $_GET['ema'];?>"  name="email"  >
                        </div>
                      </div>
                      ...
                      <div class="form-group">
                        <label class="col-sm-2 control-label">Add Offer like 10/20/30 notr use % </label>
                        <div class="col-sm-7 controls">
                          <input class="width-70" type="text" placeholder="Input"  name="product_offer"  >
                        </div>
                      </div>
                     ...
                     <button class="btn vd_btn vd_bg-green vd_white" type="submit" name="btn-save" ><i class="icon-ok"></i> Continue</button>
                     <hr>
                     <button class="btn vd_btn vd_bg-green vd_white" type="submit" name="btnfinal" ><i class="icon-ok"></i> Complete Order</button>
                    </form>
                  </div>
                </div>
                <!-- Panel Widget --> 
