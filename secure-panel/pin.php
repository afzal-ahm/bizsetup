...
$con = new DB_con();
$table = "pin";
$taxes=$con->select($table);
  
if(isset($_GET['addtaxid'])){
    if(isset($_POST['btn-save']))
    {
        ...
        $today = date('Y-m-d');
        $field_value=array($vat,$today,$location,$mode);
        $field=array('pin','date','mode','location');
        $res = $con->insert($field_value,$table,$field);
        if($res)
        {
            ?>
            <script>
                alert("New Pincode Inserted");
...
if(isset($_GET['edittaxid'])){
    $table = 'pin';
    $Taxid = $_GET['edittaxid'];
    $field_value=array($Taxid);
    $field=array('id','pin');
    $Editdata = $con->selectcatprod($table,$field_value,$field);
    if(isset($_POST['btn-save']))
    {
        $table = "pin";
        $Taxid = $_POST['taxid'];
        ...
        $mode = $_POST['mode'];
          $location = $_POST['location'];
        $field_value=array($Taxid,$vat,$mode,$location);
        $field=array('id','pin','mode','location');
        $res = $con->update($field_value,$table,$field);
        if($res)
        {
            ?>
            <script>
...
include 'config.php';
if(isset($_GET['deletetax'])){
     
    $deletetaxid = $_GET['deletetax']; 
      $delete_promo="DELETE FROM `pin` WHERE id='".$deletetaxid."'";
    $runx=mysqli_query($conn,$delete_promo);
    if($runx)
    {
        ?>
        <script>
            alert("pincode Successfully Deleted");
            window.location ='pin.php'
        </script>
        <?php
    }
...
<!-- Specific CSS -->
    ...
    <script type="text/javascript" language="JavaScript">
        jQuery(document).ready(function(){
            jQuery('#countryId').bind('change',
                function getState(e){
                    var countryId= document.getElementsByName("country_id")[0].value;
                    jQuery.ajax({
                        type: "POST",
                        url: "getstate.php",
                    ...
                    <div class="vd_content clearfix">
                        <div class="vd_title-section clearfix">
                            <div class="vd_panel-header">
                                ...
                                <small class="subtitle">List of Delivery</small> </div>
                        </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="panel-heading vd_bg-grey">
                                        <h3 class="panel-title"> <span class="menu-icon"> <i class="fa fa-bar-chart-o"></i> </span> Edit Tax </h3>
                                    </div>
                                    <div class="panel-body">';
                                        foreach($Editdata as $editkey => $editdataval)
                                       echo'<form class="form-horizontal" action="" method="post" role="form">
                                            <div class="form-group">
                                                <label class="col-sm-2 control-label">Pin </label>
                                                <div class="col-sm-7 controls">
                                                    <input class="width-70" type="number" value="'.$editdataval['pin'].'" data-toggle="tooltip" data-placement="top" name="vat">
                                                </div>
                                            </div>
                                             <div class="form-group">
                                                <label class="col-sm-2 control-label">Location </label>
                                                <div class="col-sm-7 controls">
                                                    <input class="width-70" type="number" value="'.$editdataval['location'].'" data-toggle="tooltip" data-placement="top" name="location">
                                                </div>
                                            </div>
                                             <div class="form-group">
                                                <label class="col-sm-2 control-label">payment Mode </label>
                                                <div class="col-sm-7 controls">
                                                    <input class="width-70" type="number" value="'.$editdataval['mode'].'" data-toggle="tooltip" data-placement="top" name="mode">
                                                </div>
                                            </div>
                                          
                                            <input type="hidden" name="taxid" value="'.$editdataval['id'].'">
                                            <button class="btn vd_btn vd_bg-green vd_white" type="submit" name="btn-save"><i class="icon-ok"></i> Save</button>
                                        </form>
                                    </div>
                                </div>';
                                    }elseif(isset($_GET['addtaxid'])){
                                        echo'<div class="panel widget">
                                    <div class="panel-heading vd_bg-grey">
                                        <h3 class="panel-title"> <span class="menu-icon"> <i class="fa fa-bar-chart-o"></i> </span> Add pincode </h3>
                                    </div>
                                    <div class="panel-body">
                                        <form class="form-horizontal" action="" method="post" role="form">
                                            ...
                                            <div class="form-group">
                                                <label class="col-sm-2 control-label">Delivery_price </label>
                                                <div class="col-sm-7 controls">
                                                    <input class="width-70" type="number" placeholder="Input" data-toggle="tooltip" data-placement="top" name="vat">
                                                </div>
                                            </div>
                                             <div class="form-group">
                                                <label class="col-sm-2 control-label">City area </label>
                                                <div class="col-sm-7 controls">
                                                    <input class="width-70" type="number" placeholder="City/area" data-toggle="tooltip" data-placement="top" name="location">
                                                </div>
                                            </div>
                                             <div class="form-group">
                                                <label class="col-sm-2 control-label">Payment Mode </label>
                                                <div class="col-sm-7 controls">
                                                    <input class="width-70" type="number" placeholder="online payment" data-toggle="tooltip" data-placement="top" name="mode">
                                                </div>
                                            </div>
                                     ...
                                     <div class="panel widget">
                                        <div class="panel-heading vd_bg-grey">
                                            <h3 class="panel-title" style="float: left;"> <span class="menu-icon"> <i class="fa fa-dot-circle-o"></i> </span> All Delivery </h3>
                                            <h4 class="panel-title"> <span class="menu-icon"> <i class="fa fa-dot-circle-o"></i> </span><a href="pin.php?addtaxid"> ADD Pincode</a></h4>
                                        </div>
                                        <div class="panel-body-list  table-responsive">
                                            <table class="table table-striped table-hover no-head-border"  id="data-tables">
                                                ...
                                                foreach($taxes as $key=>$taxesval){
                                                    echo'<tr>
                          
                          <td>'.$taxesval['pin'].'</td> 
                                             <td>'.$taxesval['mode'].'</td>
                                               <td>'.$taxesval['location'].'</td>         
						  <td>'.$taxesval['date'].'</td>
						 <td class="menu-action rounded-btn"><a href="showtax.php?edittaxid='.$taxesval['id'].'" data-original-title="edit" data-toggle="tooltip" data-placement="top" class="btn menu-icon vd_bg-grey"> <i class="fa fa-pencil"></i> </a> </td>
                        </tr>';}
        ...
        <div class="vd_bottom ">
            <div class="container">
                <div class="row">
                    <div class=" col-xs-12">
                        <div class="copyright">
                            Copyright &copy;2018 Rajan Admin Inc. All Rights Reserved
                        </div>
                    </div>
                </div><!-- row -->
            </div><!-- container -->
