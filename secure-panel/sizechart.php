...
if(isset($_POST['btn-save']))
{
    ...
    $size  = $_POST['size'];
    $field_value=array($product_sku,$size,$brand_size,$size_no);
    $field =array('sku','size','brand_size','size_no');
    $res_exist = $con->already_exist_sizechart($table,$field,$field_value);
    //$res = $con->insert($field_value,$table,$field);
    if($res_exist)
    {
        ?>
        <script>
            alert("size Chart uploaded");
            window.location ='sizechart.php'
        </script>
        <?php
    }
    else{
