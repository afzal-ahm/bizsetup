...
if($_FILES['file']['name'] !="" && $_FILES['file2']['name'] =="" && $_FILES['file3']['name'] =="" && $_FILES['file4']['name'] =="" ){
    $image1 = $_FILES['file']['name'];
    $image_tmp = $_FILES['file']['tmp_name'];
    move_uploaded_file($image_tmp,"productimages/$image1");
    $field_value=array($sku,$image1);
    $field =array('sku','image');
    $res = $con->update($field_value,$table,$field);
    if($res){
        echo'<script>window.location.assign("imagemanager.php?success")</script>';
    }
}elseif($_FILES['file']['name'] !="" && $_FILES['file2']['name'] !="" && $_FILES['file3']['name'] =="" && $_FILES['file4']['name'] =="" ){
    $image1 = $_FILES['file']['name'];
    $image_tmp = $_FILES['file']['tmp_name'];
    move_uploaded_file($image_tmp,"productimages/$image1");
    ...
    move_uploaded_file($image_tmp2,"productimages/$image2");

    $field_value=array($sku,$image1,$image2);
    $field =array('sku','image','images2');
    $res = $con->update($field_value,$table,$field);
    if($res){
        echo'<script>window.location.assign("imagemanager.php?success")</script>';
    }
...
elseif($_FILES['file']['name'] !="" && $_FILES['file2']['name'] !="" && $_FILES['file3']['name'] !="" && $_FILES['file4']['name'] =="" ){
    ...
    move_uploaded_file($image_tmp3,"productimages/$image3");

    $field_value=array($sku,$image1,$image2,$image3);
    $field =array('sku','image','images2','images3');
    $res = $con->update($field_value,$table,$field);
    if($res){
        echo'<script>window.location.assign("imagemanager.php?success")</script>';
    }
...
elseif($_FILES['file']['name'] =="" && $_FILES['file2']['name'] !="" && $_FILES['file3']['name'] !="" && $_FILES['file4']['name'] !="" ){
    ...
    $image_tmp4 = $_FILES['file4']['tmp_name'];
    move_uploaded_file($image_tmp4,"productimages/$image4");

    $field_value=array($sku,$image2,$image3,$image4);
    $field =array('sku','images2','images3','images4');
    $res = $con->update($field_value,$table,$field);
    if($res){
        echo'<script>window.location.assign("imagemanager.php?success")</script>';
    }
...
move_uploaded_file($image_tmp,"productimages/$image1");
 ...
 $field_value=array($sku,$image1,$image2,$image3,$image4);
...
window.location ='imagemanager.php'
</script>
<?php
}
else{
...
<?php
    ...
    echo'<script>window.location.assign("imagemanager.php?error")</script>';
}
}
...
jQuery("#result").on("click",function(e){ 
	...
	var decoded = $("#spa").html($name).text();
	
	$('#searchid').val(decoded);
	//alert(decoded);
	var skucode = $('#searchid').val();
        //alert(skucode);
	    $.ajax({
            url: "selectsubcategory.php",
            type: "POST",
    ...
    <div data-action="remove-navbar" data-original-title="Remove Navigation Bar Toggle" data-toggle="tooltip" data-placement="bottom" class="remove-navbar-button menu"> <i class="fa fa-arrows-h"></i> </div>
      ...
      <div data-action="fullscreen" data-original-title="Remove Navigation Bar and Top Menu Toggle" data-toggle="tooltip" data-placement="bottom" class="fullscreen-button menu"> <i class="glyphicon glyphicon-fullscreen"></i> </div>
          ...
          <div class="vd_title-section clearfix">
            <div class="vd_panel-header">
              <h1>Bonito Panos Dashboard</h1>
              <small class="subtitle">Bonito Panos Dashboard</small>
              ...
              <div class="col-md-12">
                <div class="panel widget">
                  <div class="panel-heading vd_bg-grey">
                    <h3 class="panel-title"> <span class="menu-icon"> <i class="fa fa-bar-chart-o"></i> </span> Add Product Images </h3>
                  </div>
                ...
                <div class="form-group" id="price1">
                        ...
                        <label class="col-sm-2 control-label">Enter Product SKU Code</label>
                        <div class="col-sm-7 controls">
                          <input  type="text" placeholder="Input" class="search" id="searchid" name="product_sku" autocomplete="off">
                        ...
                        if(isset($_GET['error'])){
                            ...
                            echo'<span style="margin-left: 174px;color: red;">*Please select all images</span>';
                        }elseif(isset($_GET['success'])){

                            echo'<span style="margin-left: 174px;color: green;">Images Uploaded successfully</span>';
                        }
                        ?>
                        <?php
    ...
    <div class="vd_bottom ">
        <div class="container">
            <div class="row">
              <div class=" col-xs-12">
                <div class="copyright">
                  	Copyright &copy;2015 Bonito Panos Inc. All Rights Reserved 
                </div>
              </div>
            </div><!-- row -->
        </div><!-- container -->
