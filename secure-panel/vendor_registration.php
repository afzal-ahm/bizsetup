              ...
              <div class="col-md-12">
                <div class="panel widget">
                  <div class="panel-heading vd_bg-grey">
                    <h3 class="panel-title"> <span class="menu-icon"> <i class="fa fa-dot-circle-o"></i> </span> view Registration</h3>
                  </div>
                  <div class="panel-body table-responsive">
                    <table class="table table-striped" id="data-tables">
                      ...
                      <tbody>
                        <?php 
                           $i=1;
 $query="SELECT * from vendor_login order by id DESC";

	$run = mysqli_query($conn,$query);
	
	while ($row=mysqli_fetch_array($run))
	{
	//	$resume =$row['catalogues'];
//print_r($res);$resume=$row['broucher'];
$resume=$row['vaccination'];

 $queryd="SELECT * from v_document where user='".$row['id']."'  ";
 $rund = mysqli_query($conn,$queryd);
 foreach($rund as $key=>$doc)
?>               	
                        <tr class="odd gradeX">
                         <td><?php echo $i++; ?></td>
                          <td><?php echo $row['username']; ?>
                          <br><span style="color:red">Service : <?php echo $row['service']; ?> </span></td>
                         <td><?php echo $row['email']; ?>
                         <br><?php echo $row['mobile']; ?></td>
                         
                         <td><?php echo $row['registration_date']; ?></td> 
                         <td><?php echo $row['type']; ?></td> 
                          <td><?php   if($row['type']=='partner') 
                          { ?>
                           <?php if($doc['permission']=='' AND $doc['bar_licence']=='' ) { ?> <p style="color:red">No document uploaded</p> <?php } ?>
                        <?php if($doc['permission']!='') { ?> <a target="_blank" href="../user/images/<?php echo $doc['permission'];?>"> View Photo Uploaded</a>
                         <br> <?php } ?>
                        <?php if($doc['bar_licence']!='') { ?>  <a target="_blank" href="../user/images/<?php echo $doc['bar_licence'];?>">View  Bar Licence/CS/CA</a>  <?php } ?>
                          <?php }
                          else
                           { ?>
                           
                           <?php if($doc['photo']=='' AND $doc['passport']==''  AND $doc['g_certificate']==''
                           AND $doc['e_bill']=='' AND $doc['affidavit']=='' AND $doc['other']==''
                           AND $doc['adhar_card']=='' AND $doc['d_l']=='' AND $doc['rent']==''
                           AND $doc['noc']=='' AND $doc['permission']==''  ) { ?> <p style="color:red">No document uploaded</p> <?php } ?>
                           
                           
                           
                         <?php if($doc['photo']!='') { ?>   <a target="_blank" href="../user/images/<?php echo $doc['photo'];?>">View  Photo Uploaded</a>
                          <br> <?php } ?>
                        <?php if($doc['passport']!='') { ?>  <a target="_blank" href="../user/images/<?php echo $doc['passport'];?>">View  Passport Uploaded</a>
                           <br> <?php } ?>
                        <?php if($doc['g_certificate']!='') { ?>  <a target="_blank" href="../user/images/<?php echo $doc['g_certificate'];?>">View  Goverment registration Certificate</a>
                           <br> <?php } ?>
                        <?php if($doc['e_bill']!='') { ?>  <a target="_blank" href="../user/images/<?php echo $doc['e_bill'];?>">View  Electricity Bill</a>
                           <br> <?php } ?>
                        <?php if($doc['affidavit']!='') { ?>  <a target="_blank" href="../user/images/<?php echo $doc['affidavit'];?>">View  Affidavit</a>
                           <br> <?php } ?>
                        <?php if($doc['other']!='') { ?>  <a target="_blank" href="../user/images/<?php echo $doc['other'];?>">View  Other Document</a>
                           <br> <?php } ?>
                         <?php if($doc['adhar_card']!='') { ?> <a target="_blank" href="../user/images/<?php echo $doc['adhar_card'];?>">View  Adhar Card</a>
                           <br> <?php } ?>
                         <?php if($doc['d_l']!='') { ?> <a target="_blank" href="../user/images/<?php echo $doc['d_l'];?>">View  Driving Licence</a>
                           <br> <?php } ?>
                         <?php if($doc['rent']!='') { ?> <a target="_blank" href="../user/images/<?php echo $doc['rent'];?>">View  Rent Agreement</a>
                           <br> <?php } ?>
                        <?php if($doc['noc']!='') { ?>  <a target="_blank" href="../user/images/<?php echo $doc['noc'];?>">View  Noc Uploaded</a>
                           <br> <?php } ?>
                        <?php if($doc['permission']!='') { ?>  <a target="_blank" href="../user/images/<?php echo $doc['permission'];?>">View  Goverment certificate</a> <?php } ?>
                         <?php  }
                           ?></td> 
                        </tr>
                       <?php } ?>
                       ...
                       if(isset($_GET['delid'])){
					   	...
					   	echo'<script>location.assign("vaccination_registration.php")</script>';
