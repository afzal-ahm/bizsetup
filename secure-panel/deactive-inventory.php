                    ...
                    <table class="table table-striped" id="data-tables">
                     ...
                     <tbody>
                       <?php
                           ...
                           $i=1;
 $query="SELECT * from product where p_copy='0' and status='0' order by id DESC";

	$run = mysqli_query($conn,$query);
					   foreach($run as $ke=>$value){
					       
					       $value['vendor_sku'];
					       $img1="SELECT * from   image_manager where  p_sku='".$value['sku']."' Limit 1";
$imge1=mysqli_query($conn,$img1);
foreach($imge1 as $key=> $image1)

    $img1x="SELECT * from   vendor_login where  code='".$value['vendor_sku']."'  ";
$imge1x=mysqli_query($conn,$img1x);
foreach($imge1x as $key=> $image1x)
  $venx=$image1x['username'];
                        ...
                        <tr>
                          ...
                          <td>
                              	<?php  if($value['producttype']=='0'){ 
                              	    ?>
                              <img style="width:50px; height:50px;" src="../images/product/<?php echo $image1['image'] ;?>">
                            <?php  } else { ?>
                                <img style="width:50px; height:50px;" src="<?php echo $image1['image'] ;?>">
                              <?php } ?>
                              <a target="_blank" href="../product-detail.php?category=<?php echo $value['category'] ;?>&sub=<?php echo $value['subcategory'] ;?>&sub_sub=<?php echo $value['subsubcategory'] ;?>&product=<?php echo $value['url'] ;?>&sku=<?php echo $value['sku'] ;?>"><?php echo $value['product_name'] ;?></a>
                          <br>
                          	<a target="_blank" href="../product-detail.php?category=<?php echo $value['category'] ;?>&sub=<?php echo $value['subcategory'] ;?>&sub_sub=<?php echo $value['subsubcategory'] ;?>&product=<?php echo $value['url'] ;?>&sku=<?php echo $value['sku'] ;?>"> click to view</a>
                          </td>
                          <td class=" "><b></b> <?php echo $value['sku'];?>
                          	<br>By-  <span style="color:red"><?php  if($value['vendor_sku']==''){ echo 'Admin';} else {   echo $venx;}?></span>
                          	<br>
                          	<?php  if($value['producttype']=='0'){ echo '<span class=" "  > <a href="javascript:void(0);" class="label label-success "  style="background-color:#4958dd;" > Single listing</a> </span>';} else { echo '<span class=" "  > <a href="javascript:void(0);" class="label label-success " style="background-color:#d8dd49;" > Bulk listing</a> </span>';}?>
                          </td>
                         
						   
                         
						  <td class="center"><b><i class="fa fa-inr" aria-hidden="true"></i></b>-<?php echo $value['mrp'];?> Offer-<?php echo $value['offer'];?>% <i class="fa fa-inr" aria-hidden="true"></i></b>-<?php echo $value['offer_amount'];?>  </td>
						  
						   
						 
						 <td>
						 
						 <span class=" " onclick="return confirm('Are you sure?')">
				<?php if(($value['featured'])=='0'){?>
        <a href="brandaction1.php?status=<?php echo $value['id'];?>" class="label label-danger " onclick="return confirm('Activate <?php echo $data?>');"> NOT </a>
<?php  }
if($value['featured']=='1'){?>
<a href="brandaction1.php?status=<?php echo $value['id'];?>" 
...
<?php  } ?> </span>
						 ...
						 <span class=" " onclick="return confirm('Are you sure?')">
				<?php if(($value['brandactive'])=='0'){?>
        <a href="featured.php?status=<?php echo $value['id'];?>" class="label label-danger " onclick="return confirm('Activate <?php echo $data?>');"> NOT </a>
<?php  }
if($value['brandactive']=='1'){?>
<a href="featured.php?status=<?php echo $value['id'];?>" 
 class="label label-success " onclick="return confirm('De-activate <?php echo $data?>');"> Yes</a>
<?php  } ?> </span>
 </td>
...
<?php  } ?> </span></td>
						 ...
						 <td class="menu-action rounded-btn"> 
						  ...
						  <a href="product-profile.php?sku=<?php echo $value['id'];?>" data-original-title="edit" data-toggle="tooltip" data-placement="top" class="btn menu-icon vd_bg-grey"> <i class="fa fa-pencil"></i> edit </a> 
						<!-- <a href="editproduct.php?sku=<?php echo $value['id'];?>" data-original-title="edit" data-toggle="tooltip" data-placement="top" class="btn menu-icon vd_bg-grey"> <i class="fa fa-pencil"></i> </a>  -->
						 <a href="deleteproduct.php?sku=<?php echo $value['id'];?>"   data-original-title="delete" data-toggle="tooltip" data-placement="top" class="btn menu-icon vd_bg-grey"> <i class="fa fa-times"></i> Delete </a>
